...
func (rh *reportHandler) ListSchemas(g *gin.Context) {
	rh.logger.Debug().Str("evt", "call ListSchemas").Msg("")

	out, gErr := rh.sc.ListSchemas(g.Request.Context(), &reportv1.ListSchemasRequest{})
	if gErr != nil {
		st, ok := errs.GRPCtoREST(gErr)
		if !ok {
			g.Set("msg", gErr.Error())
...
func (rh *reportHandler) ListTables(g *gin.Context) {
	rh.logger.Debug().Str("evt", "call ListTables").Msg("")
	...
	out, gErr := rh.sc.ListTables(g.Request.Context(), &reportv1.ListTablesRequest{
		SchemaName: schema,
	})
	if gErr != nil {
		st, ok := errs.GRPCtoREST(gErr)
...
func (rh *reportHandler) ListColumns(g *gin.Context) {
	rh.logger.Debug().Str("evt", "call ListColumns").Msg("")
	...
	out, gErr := rh.sc.ListColumns(g.Request.Context(), &reportv1.ListColumnsRequest{
		SchemaName: schema,
		TableName:  table,
	})
	if gErr != nil {
